...
!uanber
use W_Largescale_mod,       only: W_Largescale_Driver, &
                                  W_Largescale_init,   &
                                  g0_sum
!uanber
...
!---------------------------------------------------------------
! uanber:  initialize  W_Largescale_mod
!--------------------------------------------------------------   
      call w_largescale_init(axes,time)
...
!uanber
      id_tdt_W_Largescale = register_diag_field ( mod_name,       &
         'tdt_W_Largescale', axes(1:3), Time,                     &
         'temperature tendency from W_Largescale driver', &
         'K/s', missing_value=missing_value)
!uanber
      ...
      allocate (id_tracer_W_Largescale(ntp))   !uanber added
...
!uanber
        diaglname = trim(tracer_name)//  &
                    'temperature tendency from W_Largescale driver'
        id_tracer_W_Largescale(n) =    &
                         register_diag_field ( mod_name, &
                         TRIM(tracer_name)//'_W_Largescale',  &
                         axes(1:3), Time, trim(diaglname), &
                         TRIM(tracer_units)//'/s',  &
                         missing_value=missing_value)
!uanber
...
! </SUBROUTINE>
!
subroutine physics_driver_down (is, ie, js, je, npz, nq, ng,              &  !uanber added nq, ng
                                Time_prev, Time, Time_next,       &
                                lat, lon, area,                   &
                                Physics_input_block,              &
                                frac_land, rough_mom,             &
...
integer,                 intent(in)             :: is, ie, js, je, npz, nq, ng !uanber added nq, ng
...
!uanber
!note: since W_Largescale is out, you do not need to decleare it global here
      real, dimension(is:ie,js:je,npz) :: W_Largescale3D !, zfull, pfull
     ! real     :: pdt
      integer :: k
!      integer  :: nq !, ng
!      real, dimension(is-ng :ie+ng  ,js-ng  :je+ng  ,npz) :: delz, delp !, pt
!      real, intent(in), dimension(ie-is+1,je-js+1,npz) ::  delp 
!      real, dimension(is-1:ie+1 ,1:npz+1,js-1:je+1) ::pe
!      real, intent(in), dimension(is  :ie   ,1:npz+1,js  :je  ) :: peln
!      real,dimension(is-ng:ie+ng,js-ng:je+ng,npz, nq) :: q


!       real, intent(IN), dimension(:,:,:) :: delz, delp
!      real, dimension(:,:,:) ::pe
!      real, dimension(:,:,:) :: peln
!      real,dimension(:,:,:,:) :: q
...
!---------------------------------------------------------------------
      ...
      tdt => Physics_tendency_block%t_dt  !uanber: temp. tend.
      rdt => Physics_tendency_block%q_dt  !uanber: moisture tend.
...
!-----------------------------------------------------------------------
! uanber: tendency from W_Largescale
!----------------------------------------------------------------------

if (do_W_Largescale) then

      if (id_tdt_W_Largescale > 0) then
        used = send_data ( id_tdt_W_Largescale, -2.0*tdt(:,:,:), &
                           Time_next, is, js, 1)
      endif

      do n=1,ntp
        if (id_tracer_W_Largescale(n) > 0) then
          used = send_data ( id_tracer_W_Largescale(n), -2.0*rdt(:,:,:,n), &
                             Time_next, is, js, 1)
        endif
      end do
...
do k=1,npz
     write(mpp_pe()+3000,*) k,' ', tdt(is,js,k)
enddo
     call  W_Largescale_Driver(npz, is, ie, js, je, ng, nq, dt, time, hydrostatic, &
                                 t, p_full, p_half, z_full, z_half, r, tdt, rdt, pbltop, W_Largescale3D,&
                                 area)
do k=1,npz
     write(mpp_pe()+4000,*) k,' ', tdt(js,js,k)
enddo
       if (id_tdt_W_Largescale > 0) then
         used = send_data ( id_tdt_W_Largescale, +2.0*tdt(:,:,:), &
                           Time_next, is, js, 1)
...
!uanber: debug
! write(mpp_pe()+3000,*) temp_last(32,32,:)
!-----------------------------------------------------------------------
...
!---------------------------------------------------------------------
      ...
      deallocate (id_tracer_W_Largescale)  !uanber
